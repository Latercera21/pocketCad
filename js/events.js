...
//---------
            ...
            if(ne){
                ...
                // Un "lado" es la cadena completa entre puntos duros (getCurveChain),
                // no cada segmento curvo interno por separado.
                const chain = getCurveChain(fig, ne.edgeIndex);
                const alreadyIn = offsetEdges.some(o=>o.figureIndex===ne.figureIndex && o.edgeIndex===ne.edgeIndex);
                if (alreadyIn) {
                    offsetEdges = offsetEdges.filter(o => !(o.figureIndex===ne.figureIndex && chain.includes(o.edgeIndex)));
                } else {
                    if(offsetEdges.length===0) createOffsetRef(ne.figureIndex);
                    chain.forEach(ei=>{
                        if(!offsetEdges.some(o=>o.figureIndex===ne.figureIndex && o.edgeIndex===ei))
                            offsetEdges.push({figureIndex:ne.figureIndex, edgeIndex:ei});
                    });
                }
